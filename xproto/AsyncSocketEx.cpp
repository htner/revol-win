...
public:
	CAsyncSocketExHelperWindow()
	{
		...
		wndclass.lpszClassName=_T("rc_socket_wnd");
		wndclass.hIconSm=0;

		RegisterClassEx(&wndclass);

		m_hWnd=CreateWindow(_T("rc_socket_wnd"), _T("rc_socket_window"), 0, 0, 0, 0, 0, 0, 0, 0, GetModuleHandle(0));
		ASSERT(m_hWnd);
	...
	static LRESULT CALLBACK WindowProc( HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam )
	{
		...
		else if (message == WM_USER+1)
		{
			...
			CAsyncSocketEx *pSocket; int i = 0;
			for (; i<pWnd->m_nWindowDataSize; i++)
			{
			...
			if ((((LPHOSTENT)pSocket->m_pAsyncGetHostByNameBuffer)->h_addr) == NULL) {
				return 0;
			}
...
BOOL CAsyncSocketEx::Create( UINT nSocketPort /*=0*/, int nSocketType /*=SOCK_STREAM*/, long lEvent /*=FD_READ | FD_WRITE | FD_OOB | FD_ACCEPT | FD_CONNECT | FD_CLOSE*/, LPCTSTR lpszSocketAddress /*=NULL*/ )
{
...
#endif //NOLAYERS
	{
		...
		if (!Bind(nSocketPort, lpszSocketAddress))
		{
			//add test
			srand(time(NULL));
			int randomport = rand()%1000 +6000;
			if (!Bind(randomport, lpszSocketAddress))
			{
				Close();
				ASSERT(FALSE);
				return FALSE;
...
BOOL CAsyncSocketEx::Connect(LPCSTR lpszHostAddress, UINT nHostPort)
{
...
#endif //NOLAYERS
	{
		...
		if (sockAddr.sin_addr.s_addr == INADDR_NONE)
		{
			...
			memset(m_pAsyncGetHostByNameBuffer, 0, MAXGETHOSTSTRUCT);
...
int CAsyncSocketEx::Sendto(const char FAR *buf, int len, int flags, const struct sockaddr FAR *to, int tolen)
{
	return sendto(m_SocketData.hSocket, buf, len, flags, to, tolen);
}
	
int CAsyncSocketEx::Recvfrom(char FAR* buf, int len, int flags, struct sockaddr FAR *from, int FAR *fromlen)
{
	return recvfrom(m_SocketData.hSocket, buf, len, flags, from , fromlen);
}
...
BOOL CAsyncSocketEx::Accept( CAsyncSocketEx& rConnectedSocket, SOCKADDR* lpSockAddr /*=NULL*/, int* lpSockAddrLen /*=NULL*/ )
{
	...
	if (m_pFirstLayer)
	{
		//SetSockOpt(rConnectedSocket.m_SocketData);
	...
	{
		//int option = 1;
		//if (!SetSockOpt(SO_REUSEADDR, &option, sizeof(option)))
		//{
		//	int i =1;
		//}
