...
#define	MAX_RELOG	120
#define	MIN_RELOG	2
#define	MAX_RETIME	5
#define	SESS_REFRESH_TIME  (1000 * 60 * 5)
...
Login::Login(CProtoLink* xxx)
{
	...
	__rsaKey =  RSA_generate_key(512, e, NULL, NULL);
...
void Login::onLogin(XCast<PLoginRes>& cmd, nproto::connection* c)
{
	...
	if(!core::Request::ifSuccess(cmd.resCode)){
		...
		// new gateway link
		// gateway_link_.Start("192.168.50.81", 10009);
	}
}
...
void Login::rsaDecodeRc4(const std::string &ctext, std::string &ptext){
	...
	memset(buf, '1', DEF_SESSIONKEY_LENGTH);

	unsigned char ctextxxx[100];

	int num2 = RSA_public_encrypt(DEF_SESSIONKEY_LENGTH, buf, ctextxxx, __rsaKey,
		RSA_PKCS1_PADDING);


	unsigned char rc4key[100];

	std::string nstr(BN_bn2hex(__rsaKey->n));
	std::string estr(BN_bn2hex(__rsaKey->e));
	std::string aastr((const char *)ctextxxx, num2);
	std::string base64str1 = helper::Base64::encode(aastr);

	//std::string aastr2((const char *)ctextxxx, num2);
	std::string base64str2 = helper::Base64::encode(ctext);
	...
	int num = RSA_private_decrypt(ctext.length(), (const unsigned char *)ctext.c_str(), rc4key, __rsaKey,
		RSA_PKCS1_PADDING);
