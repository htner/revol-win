...
#if defined(__i386__)||defined(WIN32)||defined(LITTLE_ENDDIAN)
...
public:
	...
	Pack & push_varwstring32(const std::wstring &ws){
		size_t len = ws.size() * sizeof(wchar_t);
...
public:
	...
	std::wstring pop_varwstring32() const{
		Varstr vs = pop_varstr32_ptr();
		return std::wstring((wchar_t *)vs.data(), vs.size() / sizeof(wchar_t));
	}
...
// pair.first helper
// XXX std::map::value_type::first_type unpack
...
///////////////////////////////////////////////////////////////////////
template <class T1, class T2>
inline Pack& operator << (Pack& s, const std::map<T1, T2>& p) {
	typename std::map<T1, T2>::const_iterator it;
	s.push_uint32(p.size());
...
template <class T1, class T2>
inline const sox::Unpack& operator >> (const sox::Unpack& s, std::map<T1, T2>& p) {
	uint32_t size = s.pop_uint32();
	for (uint32_t i = 0; i < size; ++i) { 
		typename std::pair<T1, T2> kv;
...
template <class T1>
inline Pack& operator << (Pack& s, const std::list<T1>& p) {
	typename std::list<T1>::const_iterator it;
	s.push_uint32(p.size());
	for (it = p.begin(); it != p.end(); ++it) {
		s << *it;
	}
...
template <class T1>
inline const sox::Unpack& operator >> (const sox::Unpack& s, std::list<T1>& p) {
	uint32_t size = s.pop_uint32();
	for (uint32_t i = 0; i < size; ++i) { 
		T1 m;
		s >> m;	
...
template <class T1>
inline Pack& operator << (Pack& s, const std::vector<T1>& p) {
...
template <class T1>
inline const sox::Unpack& operator >> (const sox::Unpack& s, std::vector<T1>& p) {
	uint32_t size = s.pop_uint32();
	for (uint32_t i = 0; i < size; ++i) { 
		T1 m;
		s >> m;	
...
template <class T1>
inline Pack& operator << (Pack& s, const std::set<T1>& p) {
...
template <class T1>
inline const sox::Unpack& operator >> (const sox::Unpack& s, std::set<T1>& p) {
        uint32_t size = s.pop_uint32();
        for (uint32_t i = 0; i < size; ++i) {
                T1 m;
                s >> m;
