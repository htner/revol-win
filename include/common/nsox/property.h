...
//////////////////////////////////////////////
// replace 的定义： 不存则添加，存在则覆盖
//////////////////////////////////////////////

/*
typedef std::map<int, std::set<std::string> > is_map_t;

struct PBanVoice : public sox::Marshallable
{
	is_map_t us;
	...
	virtual void marshal(sox::Pack & p) const
	{
		...
		{
			std::set<std::string> const &refset=it1->second;

			p.push_uint32(uint32_t( refset.size() ) );

			for(std::set<std::string>::const_iterator it2=refset.begin(); it2!=refset.end(); it2++)
			{
	...
	virtual void unmarshal(const sox::Unpack &p)
	{
		for (uint32_t count1 = p.pop_uint32(); count1 > 0; --count1)
		{
			for (uint32_t count2 = p.pop_uint32(); count2 > 0; --count2)
			{
				...
				p >> key;
				std::set<std::string> &refset=us[key];
...
inline std::ostream & Properties::trace(std::ostream & os) const
{
	...
	{
		os << "(" << std::hex << i->first;
		os << std::dec << "," << unsigned(i->second.size());
		os << ")\n";
	}
	return os;
}
