...
inline void FixedAllocator::Chunk::Init(std::size_t blockSize, unsigned char blocks)
{
    assert(blockSize > 0);
    assert(blocks > 0);
    // Overflow check
...
inline void FixedAllocator::Chunk::Reset(std::size_t blockSize, unsigned char blocks)
{
    assert(blockSize > 0);
    assert(blocks > 0);
    // Overflow check
...
inline void FixedAllocator::Chunk::Release()
{
    delete[] pData_;
}
...
inline void* FixedAllocator::Chunk::Allocate(std::size_t blockSize)
{
...
inline void FixedAllocator::Chunk::Deallocate(void* p, std::size_t blockSize)
{
    assert(p >= pData_);
...
inline FixedAllocator::FixedAllocator(std::size_t blockSize)
...
inline FixedAllocator::FixedAllocator(const FixedAllocator& rhs)
...
inline FixedAllocator& FixedAllocator::operator=(const FixedAllocator& rhs)
{
...
inline void FixedAllocator::Swap(FixedAllocator& rhs)
{
    using namespace std;
...
inline void FixedAllocator::Deallocate(void* p)
{
...
inline FixedAllocator::Chunk* FixedAllocator::VicinityFind(void* p)
{
    assert(!chunks_.empty());
    assert(deallocChunk_);
...
inline void FixedAllocator::DoDeallocate(void* p)
{
...
inline SmallObjAllocator::SmallObjAllocator(
...
inline void* SmallObjAllocator::Allocate(std::size_t numBytes)
{
	...
	FixedAllocator xxxx(numBytes);
    Pool::iterator i = std::lower_bound(pool_.begin(), pool_.end(), xxxx, 
...
inline void SmallObjAllocator::Deallocate(void* p, std::size_t numBytes)
{
