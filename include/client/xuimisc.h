...
#define AFFIRM_BOOL2(f) if(!(f)) {return FALSE;}
...
inline static xstring Format(LPCTSTR lpszFormat, ...)
{
		...
		int len = _vsnwprintf(buffer, 256, lpszFormat, ap);
		va_end(ap);
...
inline static BOOL	TextToColor(LPCTSTR text,COLORREF& color)
{
	xstring clrstr(text);
	while(clrstr.length()<8) 
		clrstr += clrstr.substr(clrstr.length()-1,1);

	if(text[0] == '#')
	{
		DWORD dwValue = 0;
		_stscanf(text, _T("#%x"), &dwValue);
		color = dwValue;
	}
	else if (clrstr.length() > 2 && clrstr[0] == '0' && (clrstr[1] == 'x' || clrstr[1] == 'X'))
	{
		xstring tmp = clrstr;
		xstring s1 = tmp.substr(2,2);
		tmp[2] = tmp[6];
		tmp[3] = tmp[7];
		tmp[6] = s1[0];
		tmp[7] = s1[1];
		int vle = 0;
		StrToIntEx( tmp.c_str(), STIF_SUPPORT_HEX, &vle);
		color = vle;
	}
	else
	{
		std::vector<xstring> svals;
		split(xml_splitter_char,svals,text);
		AFFIRM_BOOL2(svals.size() == 3);
		color = RGB(stoi(svals[0]),stoi(svals[1]),stoi(svals[2]));
	}
	return TRUE;
}
