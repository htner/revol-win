...
#define COMMAND_SIZE (sizeof( SENDCMDOUTPARAMS ) + IDENTIFY_BUFFER_SIZE)
   ...
   for (drive = 0; drive < MAX_IDE_DRIVES; drive++)
   {
      ...
      else
      {
         ...
         {         
			...
			ULONG CommandSize = COMMAND_SIZE;
			SENDCMDINPARAMS IdInCmd = {0};			
			BYTE IdOutCmd[COMMAND_SIZE] = { 0 };  
			PSENDCMDINPARAMS Command = (PSENDCMDINPARAMS)IdOutCmd;			

	          // Retrieve the IDENTIFY data
	          // Prepare the command
			IdInCmd.cBufferSize                  = IDENTIFY_BUFFER_SIZE;   
			IdInCmd.irDriveRegs.bFeaturesReg     = 0;   
			IdInCmd.irDriveRegs.bSectorCountReg  = 1;   
			IdInCmd.irDriveRegs.bSectorNumberReg = 1;   
			IdInCmd.irDriveRegs.bCylLowReg       = 0;   
			IdInCmd.irDriveRegs.bCylHighReg      = 0;   

			// calc the drive number.   
			IdInCmd.irDriveRegs.bDriveHeadReg = (0xA0 | ( ( drive & 1 ) << 4 ));   
			//IdInCmd.irDriveRegs.bDriveHeadReg = (drive & 1) ? 0xB0 : 0xA0;

			// The command can either be IDE identify or ATAPI identify.   			
			IdInCmd.bDriveNumber = drive;   
			#define ID_CMD          0xEC            // Returns ID sector for ATA
			//IdInCmd->irDriveRegs.bCommandReg = ID_CMD;
			IdInCmd.irDriveRegs.bCommandReg = ( GetVersionParams.bIDEDeviceMap >> drive & 0x10 ) ? IDE_ATAPI_IDENTIFY : IDE_ATA_IDENTIFY;
			

			DWORD BytesReturned = 0;
	        if ( DeviceIoControl (hPhysicalDriveIOCTL, 
				                    SMART_RCV_DRIVE_DATA, 
									&IdInCmd, sizeof(SENDCMDINPARAMS),
									IdOutCmd, COMMAND_SIZE,
									&BytesReturned, NULL) )
