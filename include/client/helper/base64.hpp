...
inline string Base64::encodeFromArray(const char * data, size_t len) {
  size_t i;
  char c;
  string ret;
...
inline string Base64::encode(const string& data) {
  string::size_type i;
  char c;
...
inline string Base64::decode(const string& data) {
  string::size_type i;
  char c;
  char c1;
  string::size_type len = data.length();
