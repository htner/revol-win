...
public:
    ...
    virtual BOOL LoadIcon(LPCTSTR lpszIcon, const SIZE& size)
    {
        ...
        {
            m_sizeIcon  = size;
            m_pImage    = (IImage*)CoCreateUIObject(IID_UIImage);
    ...
    virtual BOOL LoadIcon(IImage* pImage, const SIZE& size)
    {
        ...
        if(t_bManaged)
        {
			...
			m_nxCount   = (size.cx >0 ? (UINT(m_pImage->GetSize().cx / size.cx)) : 0);
			m_nyCount   = (size.cy >0 ? (UINT(m_pImage->GetSize().cy / size.cy)) : 0);
        }
