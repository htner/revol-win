...
#include "ITimer.h"
#include "client/CriticalSectionAuto.h" 
#include "Mmsystem.h"
...
public:
	...
	void SetSuspend(BOOL b) {	m_bSuspend = b;}
...
inline static LRESULT CALLBACK  TimeWindowProc(HWND hWnd,UINT uMsg,WPARAM wParam,LPARAM lParam)
{
	...
	else if(uMsg == WM_POWERBROADCAST)//电源管理
	{
		switch(wParam)
		{
		case PBT_APMSUSPEND://休眠
			CTimer::Instance()->SetSuspend(TRUE);//ATLTRACE(_T("do Suspend\r\n"));
			break;			
		case PBT_APMRESUMEAUTOMATIC://恢复
			CTimer::Instance()->SetSuspend(FALSE);
			break;
		}
