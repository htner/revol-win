...
struct PLog: public sox::Marshallable {
...
struct PLogin : public sox::Marshallable {
	...
	virtual void marshal(sox::Pack &pk) const {
		pk << anonymous << sha1Pass << client_version << client_info;
	}
	virtual void unmarshal(const sox::Unpack &up) {
		up >> anonymous >> sha1Pass >> client_version >> client_info;
	}
};
...
struct PLoginBase : public sox::Marshallable {
	...
	virtual void marshal(sox::Pack &pk) const {
		pk << anonymous << sha1Pass << client_version << client_info << vid;
	}
	virtual void unmarshal(const sox::Unpack &up) {
		up >> anonymous >> sha1Pass >> client_version >> client_info >> vid;
	}
};
...
struct PLoginTestByAccount: public PLoginBase {
	enum {uri = (350 << 8) | protocol::LINKD_SVID};
	std::string account;
	std::string ip;
	PLoginTestByAccount(){}
	PLoginTestByAccount(const std::string& a, bool ba, const std::string &p, uint32_t v) :
		PLoginBase(ba, p, v), account(a){
	}
	virtual void marshal(sox::Pack &pk) const {
		PLoginBase::marshal(pk);
