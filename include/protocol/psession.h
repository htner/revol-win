...
#define LOGIN_KICKOFF 101
#define ADMIN_KICKOFF 102
...
struct PGetSessionMainRes  : public sox::Marshallable {
    ...
    virtual void marshal(sox::Pack &p) const {
        p << sequence << session_info << channel_info << orders << user_channel;
    }
    virtual void unmarshal(const sox::Unpack &up) {
        up >> sequence >> session_info >> channel_info >> orders >> user_channel; 
    }
};
...
struct PGetSessionUserCount: public sox::Marshallable {
    enum {uri = (324 << 8) | SESSION_SVID};
...
struct PGetSessionUserCountRes: public sox::Marshallable {
    enum {uri = (325 << 8) | SESSION_SVID};
    uint32_t count_;
...
struct PGetVoiceToken: public sox::Marshallable {
    enum {uri = (326 << 8) | SESSION_SVID};
...
struct PGetVoiceTokenRes: public sox::Marshallable {
    enum {uri = (327 << 8) | SESSION_SVID};
    std::string token_;
...
struct PGetSessionMemberCredit: public sox::Marshallable{
    enum {uri = (328 << 8) | SESSION_SVID};
    uint32_t sid_;
    uint32_t user_;
...
struct MemberInfo : public sox::Marshallable {
    uint32_t uid_;
    uint32_t credit_;
    uint32_t join_time_;

    virtual void marshal(sox::Pack &pack) const {
        pack << uid_ << credit_ << join_time_;
    }
    virtual void unmarshal(const sox::Unpack &unpack) {
        unpack >> uid_ >> credit_ >> join_time_;
    }
};

struct PGetSessionMemberCreditRes: public sox::Marshallable{
    enum {uri = (329 << 8) | SESSION_SVID};
    uint32_t sid_;
    std::vector<MemberInfo> user_credit_;

    virtual void marshal(sox::Pack &pack) const {
        pack << sid_ << user_credit_;
    }
    virtual void unmarshal(const sox::Unpack &unpack) {
        unpack >> sid_ >> user_credit_;
    }
};

struct PGetExtendChannelInfo : public sox::Marshallable{
    enum {uri = (330 << 8) | SESSION_SVID};
...
struct PGetExtendChannelInfoRes : public sox::Marshallable{
    enum {uri = (331 << 8) | SESSION_SVID};
    uint32_t channel_id_;
    sox::Properties info_;

    virtual void marshal(sox::Pack &pack) const {
        pack << channel_id_ << info_;
    }
    virtual void unmarshal(const sox::Unpack &unpack) {
        unpack >> channel_id_ >> info_;
    }
};


struct PUdpClientLogin : public PUdpLogin {
    enum {uri = (332 << 8 | protocol::SESSION_SVID)};
    uint32_t client_id_;
    virtual void marshal(sox::Pack &pak) const {
        PUdpLogin::marshal(pak);
        pak << client_id_;
    }
    virtual void unmarshal(const sox::Unpack &pak) {
        PUdpLogin::unmarshal(pak);
        pak >> client_id_;
    }
    ;
};
